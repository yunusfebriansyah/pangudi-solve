  ...
  return (
    ...
    <ReactMarkdown remarkPlugins={[remarkGfm]}
      components={{ 
        ...
        li: ({ node, ...props }) => (
          <li style={{ listStyle:"disc", marginLeft:"18px" }} {...props} />
        ),
        code: ({ children }) => (
        <Code py={2} px={4} my={2} maxW={'full'} overflowX={'auto'}>{children}</Code>
        ),
        table: ({ node, ...props }) => (
        <Table.ScrollArea borderWidth="1px" maxW="xl">
        <Table.Root showColumnBorder textStyle={'sm'}
          {...props} 
        />
        </Table.ScrollArea>
        ),
        th: ({ node, ...props }) => (
          <Table.ColumnHeader textStyle={'sm'} fontWeight={600}
            {...props} 
          />
        ),
        td: ({ node, ...props }) => (
          <Table.Cell textStyle={'sm'}
            {...props} 
          />
        )
        }}>
